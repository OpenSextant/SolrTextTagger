...
public class XmlInterpolationTest extends AbstractTaggerTest {
	...
	private String insertAnchorAtOffsets(String docText, int startOffset, int endOffset, String id) {
		...
		String insertEnd = "</A>";
		return docText.substring(0, startOffset) + insertStart + docText.substring(startOffset, endOffset) + insertEnd
	...
	private String[] analyzeReturnTokens(String docText) {
		...
		Reader filter = new HTMLStripCharFilter(new StringReader(docText), Collections.singleton("unescaped"));
		WhitespaceTokenizer ts = new WhitespaceTokenizer();
		final CharTermAttribute termAttribute = ts.addAttribute(CharTermAttribute.class);
		try {
			ts.setReader(filter);
